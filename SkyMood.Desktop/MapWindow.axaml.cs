...
using SkyMood;
// Mapsui.Styles, NetTopologySuite.Geometries, and Avalonia.Media all define a type named
// Color/Brush — aliasing every one explicitly (no bare "using Avalonia.Media;" or
// "using NetTopologySuite.Geometries;"/"using Mapsui.Styles;" name left unqualified) since a bare
// reference to any of these names would otherwise be a compile-time ambiguous-reference error.
// Same pattern as SkyMood.App/MapPage.xaml.cs — this file is a close port of that one.
...
/// <summary>
/// Native satellite/weather map — the Desktop head's port of SkyMood.App/MapPage.xaml.cs, using
/// Mapsui.Avalonia instead of Mapsui.Maui. The map/geo logic (layers, grid sampling, frame
/// playback, gradient interpolation) is copied near-verbatim since Mapsui's core API
/// (Map/Layers/Navigator/SphericalMercator) is identical across every UI front-end it ships —
/// only the surrounding widget types (Button/TextBlock/Slider/StackPanel vs their MAUI
/// equivalents) and color handling differ. Same free, keyless data sources throughout: Esri
/// World Imagery (satellite), CARTO Voyager (standard), RainViewer (real radar, past only), and
/// Open-Meteo (grid-sampled forecast heatmap, past+future).
/// </summary>
public partial class MapWindow : Window
{
    static readonly HttpClient Http = new() { Timeout = TimeSpan.FromSeconds(20) };
    const string UserAgent = "SkyMoodDesktop/1.0 (+https://soloappsstudio.com)";
    ...
    MemoryLayer? _pinLayer, _heatLayer;
    ...
    public MapWindow(GeoLocation loc, string tempDisplay, string emoji)
    {
        InitializeComponent();
        _loc = loc;
        ...
        CloseBtn.Click += (_, _) => { StopPlay(); Close(); };
        SatBtn.Click += (_, _) => SetBasemap(true);
        StdBtn.Click += (_, _) => SetBasemap(false);
        LayerBtn.Click += (_, _) => LayerMenu.IsVisible = !LayerMenu.IsVisible;
        PlayBtn.Click += (_, _) => { if (_playTimer is not null) StopPlay(); else StartPlay(); };
    ...
    static readonly AvColor GlassColor = AvColor.Parse("#33121A2E");
    ...
    void SetBasemap(bool satellite)
    {
        ...
        SatBtn.Background = new SolidColorBrush(satellite ? GlassColor : Colors.Transparent);
        SatBtn.Opacity = satellite ? 1 : 0.7;
        StdBtn.Background = new SolidColorBrush(satellite ? Colors.Transparent : GlassColor);
    ...
    void BuildLayerMenu()
    {
        ...
        foreach (var (key, def) in Layers)
        {
            ...
            btn.Click += (_, _) => { LayerMenu.IsVisible = false; _ = SetActiveLayer(key); };
    ...
    async Task SetActiveLayer(string key)
    {
        ...
        _layerKey = key;
        LayerBtn.Content = $"{def.Icon}  {def.Label}";
        StopPlay();
    ...
    void RenderLegend(LayerDef def)
    {
        ...
        brush.GradientStops.Add(new GradientStop(AvColor.FromRgb(r1, g1, b1), 0.0));
        brush.GradientStops.Add(new GradientStop(AvColor.FromRgb(r0, g0, b0), 1.0));
        LegendGrad.Background = brush;
        LegendLabels.Children.Clear();
        var inkSoft = new SolidColorBrush(AvColor.Parse("#CCDCE6F5"));
        foreach (var label in def.LegendLowToHigh.Reverse())
            LegendLabels.Children.Add(new TextBlock { Text = label, FontSize = 11, Foreground = inkSoft });
    }
    ...
    void ShowFrame(int i)
    {
        ...
        PlayBtn.Content = _playTimer is not null ? "⏸" : "▶";
    }
